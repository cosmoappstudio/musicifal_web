...
import TimeOfDayGrid from '@/components/dashboard/TimeOfDayGrid';
import TopArtistsGrid from '@/components/dashboard/TopArtistsGrid';
import TopRepeatedSongs from '@/components/dashboard/TopRepeatedSongs';
...
export default async function DashboardPage({ params }: Props) {
  ...
  const { mockAnalysis } = await import('@/lib/mock-data');

  const periodStart = new Date('2025-02-07');
  const periodEnd = new Date('2025-02-21');
  const formatDate = (d: Date) =>
    d.toLocaleDateString(locale, { day: 'numeric', month: 'short' });

  const dominantMoodKey = mockAnalysis.genres[0]?.moodKey ?? 'introspective';
